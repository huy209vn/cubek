...
impl<P: ReducePrecision> Reader<P> {
    ...
    pub fn read(&self, line_index: u32) -> (Line<P::EI>, ReduceCoordinate) {
        match self {
            Reader::Parallel(reader) => reader.read(line_index),
            Reader::Perpendicular(reader) => reader.read(line_index),
        }
    }

    pub fn len(&self) -> u32 {
        match self {
            Reader::Parallel(reader) => reader.len,
            Reader::Perpendicular(reader) => reader.len,
        }
    }
}
